...
@section('pageBar')
    <div class="kt-subheader__main">
        ...
        <div class="kt-subheader__breadcrumbs">
            ...
            <span class="kt-subheader__breadcrumbs-separator"></span>
            <a href="javascript:;" class="kt-subheader__breadcrumbs-link">Live Bookings </a>
        </div>
    </div>
                            ...
                            <tbody>
                                ...
                                @else 
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <p data-toggle="tooltip" title="{{$model->address}}" class="text-primary">
                                                {{ str_limit($model->address, 10)  }},{{$model->state}}
                                            </p>
                                        </td>
                                        <td>
                                            <p data-toggle="tooltip" title="{{$model->drop_location_address}}" class="text-primary">
                                                {{ str_limit($model->drop_location_address, 10)  }},{{$model->drop_location_state}}
                                            </p>
                                        </td>
                                        <td>{{$model->distance}}</td>
                                        <td>{{ date('d/m/Y', strtotime($model->created_at)) }}</td>
                                        <td><a href="{{ route('admin.bookings.map', $model->id) }}" class="location"><i class="flaticon2-map fa-2x" ></i></a> </td>
                                        <td>
...
@section('script')
    <script>
        $(document).ready(function(){
            ...
            $('.booking-live-menu').addClass('kt-menu__item--active');
