...
<div class="custom-container clearfix">
    ...
    <?php $collection = $model->chatRel; ?>
    <div class="chat">
        <div class="chat-header clearfix">
            <img src="{{ getImageUrl(optional($model->customerRel)->image) }}" alt="avatar" width="60" height="60" />
        
            <div class="chat-about">
                <div class="chat-with">{{ optional($model->customerRel)->name }}</div>
                <div class="chat-num-messages">already {{ count($collection) }} messages</div>
            </div>
            ...
            <ul>
                {{-- <li>
                    <div class="message-data">
                        <span class="message-data-name"><i class="fa fa-circle online"></i> {{ optional($model->customerRel)->name }}</span>
                ...
                @if(count($collection))
                    @foreach($collection as $queryModel)
                        ...
                        @if($model->customer_id != $queryModel->sender_id)
                            <li class="clearfix">
        ...
        (function(){
            var chat = {
                ...
                saveChat: function () {
                    ...
                    $.ajax({
                        url : "{{ route('admin.chats.logStore') }}",
                        method : 'post',
                        data : { 'message' : this.messageToSend, 'receiver_id' : "{{ $model->customer_id }}", 'chatId' : "{{ $model->id }}", 'receiver_id2' : "{{ $model->vendor_id }}" },
                        headers: {
